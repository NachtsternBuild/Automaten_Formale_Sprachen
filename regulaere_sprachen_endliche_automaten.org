...
*** Algorithmus zur Umwandlung NFA in DFA (verbal)

NFA:
\(A_n=(S_n, \Sigma, \delta_n, S_0^n, F_n)\),
...
*** Beispiel: Umwandlung NFA in DFA (Berechnung)

Beispiel zur Umwandlung des NFA aus Folie [[#NFA_Beispiel]] \pageref{NFA_Beispiel} in DFA:
...
*** Äquivalenz von \(\varepsilon\)FA und NFA
...
Seien \(G\) bzw. \(G_1\) und \(G_2\) Typ-3 Grammatiken. Die folgenden Probleme sind algorithmisch
entscheidbar:
